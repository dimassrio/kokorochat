...
var PHONE = window.PHONE = function(config) {
    ...
    // -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
    var PeerConnection =
        window.RTCPeerConnection(server, {optional: [{RtpDataChannels: true}]})  ||
        window.mozRTCPeerConnection(server,  {optional: [{RtpDataChannels: true}]}) ||
        window.webkitRTCPeerConnection(server,  {optional: [{RtpDataChannels: true}]});
